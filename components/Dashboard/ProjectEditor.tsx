...
const ProjectEditor: React.FC<ProjectEditorProps> = ({ user }) => {
  ...
  // Adjust textarea height to match content for smooth scrolling experience within the paper
  useEffect(() => {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-slate-100">
              ...
              {expandedChapter === chapter.title && (
                <div className="pl-6 space-y-1 py-2 animate-in slide-in-from-top-2 duration-300">
                  {chapter.sections.map((section) => (
                    <div
                      key={section}
                      className={`flex flex-col p-2.5 px-3 rounded-lg transition-all border mb-1 group/section ${
                        generating === section 
                    ...
                    >
                      <div className="flex items-center justify-between w-full">
                        <span className="text-[11px] font-medium leading-tight pr-2">
                          {section}
                        </span>
                        <button
                          onClick={(e) => {
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white relative">
        ...
        {/* Paper Container - Enforces Nigerian Formatting */}
        <div 
          ref={scrollContainerRef}
          className="flex-1 overflow-y-auto p-4 sm:p-12 bg-slate-100 scroll-smooth custom-scrollbar"
        >
          <div className="max-w-4xl mx-auto flex justify-center">
            {isLocked ? (
              ...
              >
                ...
                {/* Main Content - Strict 1 inch margins and double spacing */}
                <textarea
                  ref={textareaRef}
